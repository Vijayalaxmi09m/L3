...
**     Date/Time   : 2022-10-18, 14:49, # CodeGen: 0
**     Abstract    :
**         Main module.
			...
			for(l=0;l<10;l++){
				for(k=0;k<6;k++){
					 for(j=0;j<=9;j++)
						 ...
						 for(y=0;y<50;y++){
							trigger_PutVal(sec1);
							 clock_PutVal(~ac7segClockMux[m]);
							 for(z=0;z<0X1fFF;z++);
			
						trigger_PutVal(sec);
						 clock_PutVal(~ac7segClockMux[l]);
						 for(z=0;z<0X1fFF;z++);
				
					trigger_PutVal(min1);
					 clock_PutVal(~ac7segClockMux[k]); 
					
					 for(z=0;z<0X1fFF;z++);		 	 
					 			 trigger_PutVal(min);
					 			 clock_PutVal(~ac7segClockMux[j]); 
					 			 for(z=0;z<0X1fFF;z++);
						 }
				}
